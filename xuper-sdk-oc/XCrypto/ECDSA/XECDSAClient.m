...
- (NSData * _Nullable) signRawMessage:(NSData * _Nonnull)rawmessage keypair:(id<XCryptoKeypairProtocol> _Nonnull)keypair error:(NSError * _Nullable * _Nonnull)err {
    ...
    if ( ![keypair isKindOfClass:[XECDSAAccount class]]) {
        if (err) *err = [NSError errorWithDomain:@"keypair is not a XECDSAKey object" code:-1 userInfo:nil];
        return nil;
    }
    ...
    if ( !ecdsa_sig ) {
        if (err) *err = [NSError errorWithDomain:@"ECDSA_do_sign failure." code:-1 userInfo:nil];
        goto ErrorReturn;
    }
    
    if ( !i2d_ECDSA_SIG(ecdsa_sig, &asnisig_ref)  ) {
        if (err) *err = [NSError errorWithDomain:@"i2d_ECDSA_SIG failure." code:-1 userInfo:nil];
        goto ErrorReturn;
    }
...
- (BOOL) verifyWithPublicKey:(id<XCryptoPubKeyProtocol> _Nonnull)publickey signature:(XSignature _Nonnull)signature rawMessage:(NSData * _Nonnull)rawmessage error:(NSError * _Nullable * _Nonnull)err {
    ...
    if ( ![publickey isKindOfClass:[XECDSAPubKey class]]) {
        if (err) *err = [NSError errorWithDomain:@"keypair is not a XECDSAPubKey object" code:-1 userInfo:nil];
        return nil;
    }
...
- (BOOL) checkFormatWithAddress:(XAddress _Nonnull)address version:( unsigned int * _Nullable )version error:(NSError * _Nullable * _Nonnull)err {
    ...
    if ( !(v > 0 && v < 3) ) {
        *version = 0;
        if (err) *err = [NSError errorWithDomain:@"this cryptography has not been supported yet." code:-1 userInfo:nil];
        return false;
    }
    else if ( version ) {
        *version = v;
    ...
    if ( !memcmp(binAddress + 21, hash + (32 - 4), 4) ) {
        if (err) *err = [NSError errorWithDomain:@"invaild address." code:-1 userInfo:nil];
        return false;
    }
...
- (id<XCryptoPubKeyProtocol>)getPublicKeyFromJSON:(XJsonString)jsonString error:(NSError * _Nullable __autoreleasing *)error {
