...
- (BOOL) xuperVerifyWithPublicKeys:(NSArray<id<XCryptoPubKeyProtocol>> * _Nonnull)publickeys signature:(XSignature _Nonnull)signature rawMessage:(NSData * _Nonnull)rawmessage error:(NSError * _Nonnull * _Nullable)error;

/// 使用ECIES加密
- (NSData * _Nullable) encryptWithPublickKey:(id<XCryptoPubKeyProtocol> _Nonnull)publickey rawMessage:(NSData * _Nonnull)rawmessage error:(NSError * _Nonnull * _Nullable)error;

/// 使用ECIES解密
- (NSDate * _Nullable) decryptWithPrivateKey:(id<XCryptoPubKeyProtocol> _Nonnull)privatekey cypherRawMessage:(NSData * _Nonnull)rawmessage error:(NSError * _Nonnull * _Nullable)error;
...
- (id<XCryptoAccountProtocol> _Nullable) createNewAccountWithMnemonicLanguage:(int)language strength:(uint)strength error:(NSError * _Nonnull * _Nullable)error;

/// 创建新的账户，并导出相关文件（含助记词）到本地。生成如下几个文件：1.助记词，2.私钥，3.公钥，4.钱包地址
- (id<XCryptoAccountProtocol> _Nullable) exportNewAccountWithMnemonicLanguage:(int)languag strength:(uint)strength error:(NSError * _Nonnull * _Nullable)error;

/// 从助记词恢复钱包账户
- (id<XCryptoAccountProtocol> _Nullable) retrieveAccountByMnemonic:(NSString * _Nonnull)mnemonic language:(int)language error:(NSError * _Nonnull * _Nullable)error;

/// 从助记词恢复钱包账户，并用支付密码加密私钥后存在本地，
/// 返回的字段：（随机熵（供其他钱包软件推导出私钥）、助记词、私钥的json、公钥的json、钱包地址） as ECDSAAccount，以及可能的错误信息
- (id<XCryptoKeyInfo> _Nullable) retrieveAccountByMnemonicAndSavePrivKeyToPath:(NSString * _Nonnull)path language:(int)language mnemonic:(NSString * _Nonnull)mnemoni password:(NSString * _Nonnull)password error:(NSError * _Nonnull * _Nullable)error;
...
- (NSData * _Nullable) generateSeedErrorCheckingWithMnemonic:(NSString * _Nonnull)mnemonic password:(NSString * _Nonnull)password keylen:(int)keylen language:(int)language error:(NSError * _Nonnull * _Nullable)error;
...
/// 从导出的私钥文件读取私钥的byte格式
- (NSData * _Nullable) getBinaryPrivateKeyFromFilePath:(NSString * _Nonnull)path password:(NSString * _Nonnull)password error:(NSError * _Nonnull * _Nullable)error;
...
/// 使用支付密码从导出的私钥文件读取私钥
- (id<XCryptoAccountProtocol> _Nullable) getPrivateKeyFromFilePath:(NSString * _Nonnull)path password:(NSString * _Nonnull)password error:(NSError * _Nonnull * _Nullable)error;

/// 从二进制加密字符串获取真实私钥的byte格式
- (id<XCryptoAccountProtocol> _Nullable) getBinaryPrivateKeyFromString:(NSString * _Nonnull)string password:(NSString * _Nonnull)password error:(NSError * _Nonnull * _Nullable)error;
...
/// 产生随机熵
- (NSData * _Nullable) generateEntropyWithBitSize:(int)bitSize error:(NSError * _Nonnull * _Nullable)error;
...
// 从导出的公钥文件读取公钥
- (id<XCryptoPubKeyProtocol> _Nullable) getPublicKeyFromJSON:(XJsonString _Nonnull)jsonString error:(NSError * _Nonnull * _Nullable)error;
...
- (NSData * _Nullable) getSharedPublicKeyForPublicKeys:(NSArray<id<XCryptoPubKeyProtocol>>* _Nonnull)keys error:(NSError * _Nonnull * _Nullable)error;
...
- (NSData * _Nullable) generateMultiSignSignatureWithS:(NSData * _Nonnull)s r:(NSData * _Nonnull)r error:(NSError * _Nonnull * _Nullable)error;

/// VerifyMultiSig 使用ECC公钥数组来进行多重签名的验证
- (NSData * _Nullable) verifyMultiSigWithPubkeys:(NSArray<id<XCryptoPubKeyProtocol>> * _Nonnull)keys signature:(NSData * _Nonnull)signature message:(NSData * _Nonnull)message error:(NSError * _Nonnull * _Nullable)error;

/// MultiSign  多重签名的另一种用法，适用于完全中心化的流程
/// 使用ECC私钥数组来进行多重签名，生成统一签名格式
- (NSData * _Nullable) multiSignWithPrivateKeys:(NSArray<id<XCryptoAccountProtocol>> * _Nonnull)keys message:(NSData * _Nonnull)message error:(NSError * _Nonnull * _Nullable)error;
